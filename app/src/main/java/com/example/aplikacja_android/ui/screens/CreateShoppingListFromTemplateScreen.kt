...
@Composable
fun CreateShoppingListFromTemplateScreen(navController: NavController, shoppingList: ShoppingList){
        ...
        Button(
            onClick = {
                if (startDate != null && endDate != null) {
                    GlobalScope.launch{
                        val newShoppingList = shoppingList.copy(id = 0, name = "Shopping List from $startDate to $endDate", isTemplate = false)
                        val newShoppingListId = databaseViewModel.createShoppingList(newShoppingList)
                        val originalShoppingItems = databaseViewModel.getItemsForShoppingList(shoppingList.id).asFlow().firstOrNull() ?: emptyList()

                        originalShoppingItems.forEach { item ->
                            val newShoppingItem = item.copy(itemId = 0, listId = newShoppingListId.toInt())

                            databaseViewModel.createShoppingItem(newShoppingItem)
                        }
                    }
